...
aws-update-fn:
	aws --profile=$(AWS_PROFILE) lambda update-function-code --region $(AWS_REGION) --function-name alertdispatcher-$(ALERT_NAME) --zip-file fileb://./alertdispatcher-$(ALERT_NAME).zip
...
install: build zip aws-install-fn cleanup aws-subscribe-sns aws-lambda-invoke-sns aws-test-integration
	@echo ">> λ function created"

update: build zip aws-update-fn cleanup aws-test-integration
